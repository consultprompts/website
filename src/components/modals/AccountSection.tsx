...
const label = 'text-[15px] font-bold m-0';
    ...
    <div className="flex flex-col h-full min-h-0">
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-4 md:py-6">
        <div className="w-full max-w-[1000px] mx-auto px-4 settings:px-8">
          ...
          {/* Identity card */}
          <div className="gradient-card my-4 sm:my-6 rounded-2xl border border-white/10 py-4 sm:py-6 px-4 sm:px-6 flex items-center gap-4">
            ...
            <div className="min-w-0">
              <h3 className="font-display font-bold italic text-lg m-0 truncate">
                {user.displayName || 'Prophet'}
              </h3>
              <p className="text-ink-muted text-[13px] m-0 truncate">
                {user.email}
              </p>
          ...
          {/* Details grid */}
          <div className={`p-4 sm:p-6 grid grid-cols-1 sm:grid-cols-5 gap-10`}>
            <div>
              <p className={label}>
                Email Status
              </p>
              {user.emailVerified ? (
                <span className="mt-1 inline-flex items-center gap-1.5 text-[13px] font-bold" style={{ color: 'var(--color-brand-primary)' }}>
                  <CheckCircle2 className="w-4.5 h-4.5" /> Verified
                </span>
              ) : (
                <span className="inline-flex items-center gap-1.5 text-[13px] font-bold" style={{ color: '#F5C542' }}>
                  <MailWarning className="w-4.5 h-4.5" /> Not verified
                </span>
              )}
            </div>
            {memberSince && (
            ...
            <div>
              ...
              <p className="text-[13px] mt-1 uppercase">
                {user.roles.join(', ') || 'member'}
              </p>
            </div>
          </div>
          ...
          {/* Preferences */}
          <div className={`p-4 sm:p-6 flex items-center gap-4 flex-wrap`}>
            <div>
              <p className={label}>
                Appearance
              </p>
          ...
          {!user.emailVerified && (
            ...
            >
              <div>
                <p className={label} style={{ color: '#F5C542' }}>
                  Your email isn't verified yet
                </p>
                <p className="text-ink-muted text-[12px] mt-1 m-0">
                  Verify it to unlock all account features.
                </p>
              </div>
              <CustomButton
          ...
          {/* Security */}
          <div className={`p-4 sm:p-6 flex items-center gap-10 flex-wrap`}>
            <div>
              <p className={label}>
                Password</p>
              <p className="text-ink-muted text-[12px] mt-1 m-0">
